...
import { useSysdesStore } from "@/store/sysdesStore";
...
export function isSysdesPhaseComplete(
  phase: number,
  progress: Record<number, AiePhaseProgress>
): boolean {
  const meta = getSysdesPhase(phase);
  if (!meta) return false;
  const p = progress[phase];
  return !!p?.done;
}

export function computeSysdesOverallProgress(progress: Record<number, AiePhaseProgress>): {
  done: number;
  total: number;
  pct: number;
} {
  const total = SYSDES_PHASES.length;
  const done = SYSDES_PHASES.filter((p) => isSysdesPhaseComplete(p.num, progress)).length;
  return { done, total, pct: Math.round((done / total) * 100) };
}

export function findCurrentSysdesPhase(progress: Record<number, AiePhaseProgress>): number {
  for (const p of SYSDES_PHASES) {
    if (!isSysdesPhaseComplete(p.num, progress)) return p.num;
  }
  return SYSDES_PHASES[SYSDES_PHASES.length - 1].num;
}

export function useSysdes() {
  const { user } = useAuthStore();
  const store = useSysdesStore();
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      const rows = await db.select<AieProgressRow[]>(
        "SELECT * FROM system_design_progress WHERE user_id = ?",
        [user!.id]
      );
      ...
      if (store.selectedPhase === 1) {
        const next = findCurrentSysdesPhase(map);
  ...
  async function persist(
    ...
    await db.execute(
      `INSERT INTO system_design_progress (user_id, phase_num, done, section_index, notes, completed_at, updated_at)
