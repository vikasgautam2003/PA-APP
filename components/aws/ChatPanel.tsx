...
import type { AwsDay, AwsChatMessage, AwsImageResult, AwsVideoResult } from "@/types";
import { useAwsChat } from "@/hooks/useAwsChat";
import { useSettingsStore } from "@/store/settingsStore";

interface Props {
  day: AwsDay | undefined;
  open: boolean;
  onToggle: () => void;
}
  ...
  return (
    ...
    >
      <span style={{
        ...
        borderRadius: "50%",
        background: "radial-gradient(circle, rgba(255,153,0,0.55) 0%, rgba(32,116,217,0.30) 40%, transparent 70%)",
        filter: "blur(8px)",
        animation: "aws-orb-halo 3.6s ease-in-out infinite",
        pointerEvents: "none",
      }} />
      <span style={{
        position: "absolute", inset: 0,
        borderRadius: "50%",
        background: "conic-gradient(from 0deg, #ff9900, #ffb84d, #2074d9, #00b8d4, #ff9900)",
        animation: "aws-orb-spin 6s linear infinite",
        boxShadow: "0 12px 36px rgba(255,153,0,0.45), 0 4px 14px rgba(32,116,217,0.4)",
      }} />
      <span style={{
        position: "absolute", inset: 4,
        borderRadius: "50%",
        background: "radial-gradient(circle at 30% 25%, rgba(255,255,255,0.20) 0%, rgba(35,47,62,0.65) 75%)",
        backdropFilter: "blur(8px)",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 22, color: "#fff",
        textShadow: "0 0 12px rgba(255,153,0,0.6)",
      }}>
        {open ? "✕" : "AWS"}
      </span>
    </button>
...
function Overlay({ day, onClose }: { day: AwsDay | undefined; onClose: () => void }) {
  const { messages, sending, error, sendMessage, clearThread } = useAwsChat(day);
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 40px 120px rgba(0,0,0,0.75), 0 0 0 1px var(--aws-orange-glow)",
      ...
      >
        <header style={{
          ...
          background: "linear-gradient(135deg, var(--aws-orange-soft) 0%, rgba(32,116,217,0.08) 50%, transparent 100%)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--aws-orange)", marginBottom: 2 }}>
                AWS Tutor · Day {day ? String(day.day).padStart(2, "0") : "—"}
              </p>
              <p style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.01em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {day?.kind === "review" ? "Phase review" : day?.kind === "capstone" ? "Capstone" : day?.topic ?? ""}
              </p>
            </div>
          </div>
        ...
        <div style={{
          ...
          <textarea
            ...
            placeholder={day ? `Ask anything about ${day.topic ?? "this day"}…` : "Ask…"}
            rows={2}
            style={{
            ...
            onFocus={(e) => { e.target.style.borderColor = "var(--aws-orange)"; e.target.style.boxShadow = "0 0 0 4px var(--aws-orange-glow)"; }}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            <button
              ...
              disabled={!serpapiKey}
              title={serpapiKey ? "Toggle web search" : "Add SerpAPI key in Settings"}
              style={{
                display: "flex", alignItems: "center", gap: 8,
                padding: "8px 14px", borderRadius: 999,
                border: useWeb && serpapiKey ? "1px solid var(--aws-orange)" : "1px solid var(--border)",
                background: useWeb && serpapiKey
                  ? "var(--aws-orange-soft)"
                  : "transparent",
                color: useWeb && serpapiKey ? "var(--aws-orange)" : serpapiKey ? "var(--text-secondary)" : "var(--text-faint)",
            ...
            <button
              ...
              style={{
                ...
                boxShadow: !input.trim() || sending || !groqKey ? "none" : "0 6px 20px var(--aws-orange-glow)",
                transition: "all 0.22s cubic-bezier(0.34, 1.56, 0.64, 1)",
              }}
            >
              Ask ↑
...
function EmptyState({ dayTopic }: { dayTopic?: string }) {
  const suggestions = dayTopic ? [
    `Explain ${dayTopic} with a worked CLI example`,
    `What's the SAA-C03 trap on ${dayTopic}?`,
    `Show me a video tutorial on ${dayTopic}`,
    `Compare ${dayTopic} to its closest AWS alternative`,
  ] : [];
  return (
    <div style={{ padding: "60px 0", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
      <div style={{
        ...
        background: "conic-gradient(from 0deg, #ff9900, #ffb84d, #2074d9, #00b8d4, #ff9900)",
        animation: "aws-orb-spin 6s linear infinite",
        boxShadow: "0 0 60px var(--aws-orange-glow)",
      }} />
      <div>
        <h3 style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.02em", marginBottom: 6 }}>
          Ask anything about today&apos;s AWS topic.
        </h3>
        <p style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: 1.6, maxWidth: 460, margin: "0 auto" }}>
          I&apos;ll explain in depth with CLI / IaC examples, exam traps, and real-world scenarios. Toggle web search for current docs, videos, and architecture diagrams.
        </p>
      </div>
...
function Bubble({ msg }: { msg: AwsChatMessage }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: isUser ? "0 4px 18px var(--aws-orange-glow)" : "none",
      }}>
        <ReactMarkdown
          components={{
            ...
            h3: ({ children }) => <h4 style={{ fontSize: 13.5, fontWeight: 700, margin: "10px 0 5px", color: isUser ? "#fff" : "var(--aws-orange)" }}>{children}</h4>,
            hr: () => <hr style={{ border: "none", borderTop: "1px solid var(--border)", margin: "12px 0" }} />,
            a: ({ children, href }) => (
              <a href={href} target="_blank" rel="noopener noreferrer" style={{
                color: isUser ? "#fff" : "var(--aws-orange)",
            ...
            code: ({ children, className }) => {
              ...
              if (isBlock) {
                return (
                  <pre style={{
                    ...
                    borderRadius: 10,
                    background: isUser ? "rgba(0,0,0,0.30)" : "var(--aws-pdf-bg)",
                    color: isUser ? "#fff" : "#e6edf3",
                    fontSize: 12.5, lineHeight: 1.6,
                    fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
                    overflowX: "auto",
                    border: isUser ? "1px solid rgba(255,255,255,0.18)" : "1px solid var(--aws-navy)",
                  }}>
                    <code>{children}</code>
                  </pre>
                );
              ...
              return (
                <code style={{
                  fontSize: 12.5,
                  fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
                  background: isUser ? "rgba(255,255,255,0.18)" : "var(--aws-orange-soft)",
                  color: isUser ? "#fff" : "var(--aws-orange)",
            ...
            blockquote: ({ children }) => (
              <blockquote style={{
                borderLeft: `3px solid ${isUser ? "rgba(255,255,255,0.5)" : "var(--aws-orange)"}`,
            ...
            th: ({ children }) => <th style={{ border: "1px solid var(--border)", padding: "6px 10px", textAlign: "left", background: "var(--aws-orange-soft)", color: "var(--aws-orange)" }}>{children}</th>,
...
function VideoStrip({ videos }: { videos: AwsVideoResult[] }) {
  return (
    <section>
        ...
        {videos.map((v, i) => (
          <a key={i} href={v.link} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow = "0 8px 24px var(--aws-orange-glow)";
            }}
            onMouseLeave={(e) => {
            ...
            <div style={{ position: "relative", aspectRatio: "16/9", background: "var(--bg-hover)" }}>
              ...
              <span style={{
                ...
                <span style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: "var(--aws-orange)", fontSize: 14,
...
function ImageStrip({ images }: { images: AwsImageResult[] }) {
  return (
    <section>
      ...
      <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
        {images.map((img, i) => (
          <a key={i} href={img.link} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.04)"; e.currentTarget.style.borderColor = "var(--aws-orange)"; }}
...
function WebStrip({ web }: { web: import("@/types").AwsWebResult[] }) {
  return (
    <section>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {web.map((w, i) => (
          <a key={i} href={w.link} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--aws-orange)"; e.currentTarget.style.transform = "translateX(2px)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.transform = "translateX(0)"; }}
          >
            <p style={{ fontSize: 11.5, fontWeight: 600, color: "var(--aws-orange)", marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              [W{i + 1}] {w.title}
            </p>
...
function Notice({ tone, children }: { tone: "warn" | "error" | "info"; children: React.ReactNode }) {
  const palette = tone === "error"
    ...
    : tone === "info"
    ? { c: "var(--aws-orange)", bg: "var(--aws-orange-soft)" }
